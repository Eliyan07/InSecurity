...
const translateDetail = (detail: string, t: (key: string, opts?: Record<string, string | number>) => string): string => {
  ...
  const fixed: Record<string, string> = {
    ...
    'External media auto-scan enabled': 'audit.details.externalMediaAutoScanEnabled',
    'External media auto-scan disabled': 'audit.details.externalMediaAutoScanDisabled',
  ...
  const patterns: [RegExp, string, (m: RegExpMatchArray) => Record<string, string>][] = [
    ...
    [/^External media detected: (.+)$/, 'audit.details.externalMediaDetected', m => ({ path: m[1] })],
    [/^External media scan started: (.+)$/, 'audit.details.externalMediaScanStarted', m => ({ path: m[1] })],
